...
# This runs after ClickHouse starts

set -e

# Wait for ClickHouse to be ready
until clickhouse-client --user flowlike --password flowlike_test --query "SELECT 1" > /dev/null 2>&1; do
    echo "Waiting for ClickHouse..."
    sleep 1
done

echo "Creating flowlike_test database..."
clickhouse-client --user flowlike --password flowlike_test --query "CREATE DATABASE IF NOT EXISTS flowlike_test"

echo "Creating tables..."
clickhouse-client --user flowlike --password flowlike_test --database flowlike_test --query "
...
) ENGINE = MergeTree() ORDER BY (id, order_date)
"
...
clickhouse-client --user flowlike --password flowlike_test --database flowlike_test --query "
INSERT INTO orders (id, user_id, product_name, quantity, price, status) VALUES
